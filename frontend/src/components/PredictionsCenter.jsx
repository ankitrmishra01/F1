              ...
              <select value={season} onChange={(e) => setSeason(Number(e.target.value))}>
                {availableSeasons.map((y) => (
                  <option key={y} value={y}>
                    {y} Season {y < 2025 ? "(Completed Real Database GP)" : "(Telemetry ML Model)"}
                  </option>
                ))}
              </select>
            </div>
              ...
              {racePrediction.is_completed ? (
                <div className="dual-hero-banner">
                  ...
                  <div className="hero-card ml-card">
                    <span className="card-tag">🤖 ML MODEL PREDICTED FAVOURITE &middot; {racePrediction.race_name}</span>
